...
import AccidentMapTimeline, { type MapAccident } from '@/components/AccidentMapTimeline'

type Accident = MapAccident & {
  manufacturer?: string
    ...
    <div className="operator-table">
      ...
      {rows.map((row) => (
        <div className="operator-table-row" key={row.operator}>
          <strong>{row.operator}</strong><span>{row.accidents}</span><span>{row.fatalAccidents}</span><span>{row.fatalities.toLocaleString('tr-TR')}</span><span>{ageText(row.avgAccidentAircraftAgeYears)}</span>
        </div>
      ))}
    </div>
  )
...
export default function AviationAccidentsDrawer() {
  ...
  const [mode, setMode] = useState<'dashboard' | 'map' | 'cases'>('dashboard')
  ...
  const dashboardRows: FamilySummary[] = useMemo(() => {
    if (scope === 'ntsb' && ntsb.types.length) {
      return ntsb.types.slice(0, 30).map((x) => ({ family: x.family, accidents: x.accidents, fatalAccidents: x.fatalAccidents, fatalities: x.fatalities, avgAgeYears: x.avgAgeYears, minAgeYears: x.minAgeYears, maxAgeYears: x.maxAgeYears }))
    }
  ...
  const scopeCoverage = scope === 'ntsb' ? ntsb.coverageNote : scope === 'discovery' ? (payload.discoveryNote || 'Secondary global discovery records awaiting official-source verification.') : payload.coverageNote
      ...
      <aside className={open ? 'info-drawer open accidents-drawer safety-dashboard-drawer' : 'info-drawer accidents-drawer safety-dashboard-drawer'}>
        <div className="drawer-head">
          <div>
            ...
            <p>Resmî global indeks · dünya haritası · 1970–2026 zaman çizelgesi · NTSB bulk · filo yaşı</p>
          </div>
        ...
        <div className="safety-mode-tabs">
          ...
          <button className={mode === 'map' ? 'active' : ''} onClick={() => { setMode('map'); setSelected(null); setSelectedFamily(null) }}>Harita & Zaman</button>
        ...
        {mode === 'map' && (
          <AccidentMapTimeline
            items={payload.items}
            mapNote={payload.mapNote}
            onOpenCase={(item) => { setSelected(item as Accident); setMode('cases') }}
          />
        )}
        ...
        {mode === 'dashboard' && !selectedFamily && (
          <>
            ...
            {scope === 'ntsb' && (ntsb.causeCategories || []).length > 0 && <section className="dashboard-block">
            ...
            {scope === 'ntsb' && (ntsb.operatorStats || []).length > 0 && <section className="dashboard-block">
            ...
            <div className="type-ranking">
              <div className="dashboard-section-title"><span>Aircraft type ranking</span><small>{scope === 'ntsb' ? 'NTSB kapsamı — global toplam değil' : scope === 'discovery' ? 'Secondary discovery — doğrulama bekliyor' : 'Official-source federated index'}</small></div>
              {dashboardRows.map((row) => {
                const current = currentProfiles.get(row.family)
                const legacy = legacyProfiles.get(row.family)
                return <button className="type-row" key={`${scope}-${row.family}`} onClick={() => setSelectedFamily(row.family)}>
                  <div className="type-row-main"><strong>{row.family}</strong><span>{scope === 'ntsb' ? `${row.accidents} NTSB olay · ${row.fatalAccidents} fatal · ${row.fatalities.toLocaleString('tr-TR')} NTSB-scope ölüm` : scope === 'discovery' ? `${row.accidents} discovery kayıt · ölüm verisi varsa toplam ${row.fatalities.toLocaleString('tr-TR')}` : `${row.accidents} official-index olay · ${row.fatalAccidents} fatal · ${row.fatalities.toLocaleString('tr-TR')} ölüm`}</span></div>
        ...
        {mode === 'dashboard' && selectedFamily && <div className="type-detail-dashboard">
          <button className="back-link" onClick={() => setSelectedFamily(null)}>← Tip sıralamasına dön</button>
          <div className="eyebrow">AIRCRAFT TYPE SAFETY PROFILE</div><h3>{selectedFamily}</h3>
          ...
          {(activeNtsb?.causeCategories || []).length > 0 && <section><div className="dashboard-section-title"><span>Bu tipte neden / katkı faktörleri</span><small>NTSB Findings · multi-label</small></div><FactorBars rows={(activeNtsb?.causeCategories || []).slice(0, 9)} /><p className="method-note">Aynı accident event birden fazla faktörde sayılabilir; bu yüzden kategoriler toplamı %100 olmak zorunda değildir.</p></section>}
          {(activeNtsb?.operatorStats || []).length > 0 && <section><div className="dashboard-section-title"><span>Bu tipte geçmiş operator kayıtları</span><small>NTSB kapsamı</small></div><OperatorTable rows={(activeNtsb?.operatorStats || []).slice(0, 12)} /></section>}
          <section className="fleet-snapshot">
            ...
            {activeFleet ? <>
              ...
              {activeFleet.operatorSnapshots.length > 0 && <div className="fleet-age-table"><div className="fleet-age-head"><span>Havayolu</span><span>Tip / alt tip</span><span>Adet</span><span>Ort. yaş</span><span>Kaynak tarihi</span></div>{activeFleet.operatorSnapshots.map((row) => <a className="fleet-age-row" href={row.sourceUrl} target="_blank" rel="noreferrer" key={`${row.airline}-${row.variant}`}><strong>{row.airline}</strong><span>{row.variant}</span><span>{row.fleetCount}</span><span>{row.avgAgeYears.toFixed(1)} yıl</span><span>{row.sourceUpdatedAt}</span></a>)}</div>}
              {activeFleet.note && <p>{activeFleet.note}</p>}<p className="method-note">{fleet.methodNote}</p>
            </> : legacyFleet ? <><strong className="fleet-status">{legacyFleet.status}</strong><div className="operator-chips">{legacyFleet.currentOperators.map((x) => <span key={x}>{x}</span>)}</div>{legacyFleet.note && <p>{legacyFleet.note}</p>}{legacyFleet.sourceUrl && <a href={legacyFleet.sourceUrl} target="_blank" rel="noreferrer">Fleet kaynağını aç ↗</a>}</> : <p>Bu uçak tipi için güncel operatör/yaş snapshot’ı henüz eklenmedi.</p>}
          </section>
          <section><h4>İndeksteki ilgili olaylar</h4>{activeCases.length ? activeCases.map((item) => <button className="accident-mini-row" key={item.id} onClick={() => { setSelected(item); setMode('cases') }}><span>{item.date}</span><strong>{item.title}</strong><small>{item.sourceTier === 'discovery' ? 'DISCOVERY' : item.aircraftAgeYears != null ? `~${item.aircraftAgeYears.toFixed(1)} yaş · ${item.fatalities ?? 0} ölüm` : `${item.fatalities ?? 0} ölüm`}</small></button>) : <p>Bu tip için henüz vaka kartı yok.</p>}</section>
        </div>}

        {mode === 'cases' && selected ? <div className="accident-detail">
          <button className="back-link" onClick={() => setSelected(null)}>← Listeye dön</button>
          <div className="news-meta"><span>{selected.date}</span><span>{selected.authority}</span><span>{selected.status}</span><span>{selected.sourceTier === 'discovery' ? 'DISCOVERY · VERIFY' : 'OFFICIAL INDEX'}</span></div>
          <h3>{selected.title}</h3>
          <div className="accident-grid">
            {selected.aircraft && <div><small>Aircraft</small><strong>{selected.aircraft}</strong></div>}{selected.family && <div><small>Family</small><strong>{selected.family}</strong></div>}{selected.operator && <div><small>Operator</small><strong>{selected.operator}</strong></div>}{selected.location && <div><small>Location</small><strong>{selected.location}</strong></div>}{typeof selected.aircraftAgeYears === 'number' && <div><small>Approx. aircraft age</small><strong>{selected.aircraftAgeYears.toFixed(1)} yıl</strong></div>}{typeof selected.fatalities === 'number' && <div><small>Fatalities</small><strong>{selected.fatalities}</strong></div>}{typeof selected.survivors === 'number' && <div><small>Survivors</small><strong>{selected.survivors}</strong></div>}
          </div>
          ...
          <section><h4>Olay özeti</h4><p>{selected.summary}</p></section>{selected.probableCause && <section><h4>Probable cause / bulgular</h4><p>{selected.probableCause}</p></section>}
          <div className="accident-links">{selected.reportUrl && <a href={selected.reportUrl} target="_blank" rel="noreferrer">Resmî rapor ↗</a>}<a href={selected.sourceUrl} target="_blank" rel="noreferrer">{selected.sourceTier === 'discovery' ? 'Discovery kaynağı ↗' : 'Soruşturma kaynağı ↗'}</a>{selected.discoveryArticleUrl && <a href={selected.discoveryArticleUrl} target="_blank" rel="noreferrer">Olay sayfası ↗</a>}<a href={documentarySearch(selected)} target="_blank" rel="noreferrer">Video / belgesel ara ↗</a></div>
        </div> : mode === 'cases' ? <>
          <div className="accident-coverage">{payload.coverageNote}</div>
          <div className="cert-controls accident-controls"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Uçuş, uçak, aile, operatör, ülke..." /><select value={year} onChange={(e) => setYear(e.target.value)}>{years.map((x) => <option key={x}>{x}</option>)}</select></div>
          <div className="news-list">{filtered.map((item) => <button className="accident-card" key={item.id} onClick={() => setSelected(item)}><div className="news-meta"><span>{item.date}</span><span>{item.authority}</span><span>{item.sourceTier === 'discovery' ? 'DISCOVERY' : 'OFFICIAL'}</span>{item.aircraftAgeYears != null && <span>~{item.aircraftAgeYears.toFixed(1)} yaş</span>}</div><h3>{item.title}</h3><p>{[item.aircraft, item.operator, item.location].filter(Boolean).join(' · ')}</p><div className="accident-card-foot"><span>{item.status}</span>{typeof item.fatalities === 'number' && <span>{item.sourceTier === 'discovery' ? `${item.fatalities} reported deaths` : `${item.fatalities} fatality`}</span>}</div></button>)}</div>
        </> : null}

        <div className="drawer-foot">Global indeks: {new Date(payload.updatedAt).toLocaleString('tr-TR')} · Official: {officialItems.length} · Discovery: {discoveryItems.length} · Map coords: {payload.counts?.withCoordinates ?? 'yenileniyor'} · NTSB stats: {ntsb.totalAccidents ? new Date(ntsb.generatedAt).toLocaleString('tr-TR') : 'ilk üretim bekleniyor'} · Fleet snapshot: {fleet.profiles.length ? new Date(fleet.updatedAt).toLocaleString('tr-TR') : 'yüklenmedi'}</div>
      </aside>
    </>
  )
}
